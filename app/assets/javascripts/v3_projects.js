...
$(document).ready(function() {
  ...
  function check_all() {
    ...
    var inst_id_list = $('.v3_instance_id_list').attr("v3_instance_ids").split(",");
    ...
    $.getJSON("/v3_projects/" + proj_id + "/check_deployed", function(result) {
      ...
      for (i = 0; i < inst_id_list.length; i++) {
        ...
        if (result[inst_id] === "deployed") {
          ...
          var row=$('.instance_row[v3_instance_id="' + inst_id + '"]');
  ...
  function reachable_check(inst_id) {
    $('.reachable_button_content[v3_instance_id="' + inst_id + '"]').replaceWith('<div class="reachable_button_content" v3_instance_id="' + inst_id + '"><img src="/assets/ajax-loader.gif" title="Working..." /></div>')
    console.log("Checking reachability for instance id:" + inst_id);
    $.getJSON("/v3_instances/" + inst_id + "/reachable", function(result){
      console.log("Got result for " + inst_id + ": " + result.reachable);
      if (result.reachable === "true") {
        $('.reachable_button_content[v3_instance_id="' + inst_id + '"]').replaceWith('<div class="reachable_button_content" v3_instance_id="' + inst_id + '"><span class="text-success glyphicon glyphicon-ok" title="Success!"></span></div>')
      } else {
        $('.reachable_button_content[v3_instance_id="' + inst_id + '"]').replaceWith('<div class="reachable_button_content" v3_instance_id="' + inst_id + '"><span class="text-danger glyphicon glyphicon-remove" title="' + result.error + '"></span></div>')
      }
    }); 
  }
  ...
  $('.reachable_button').click(function() {
    var inst_id = $(this).attr("v3_instance_id");
  ...
  $('.edit_instance_check_box_no_openshift').change(function(){
    ...
    console.log(inst_id);
    if (this.checked) {
      $('.edit_instance_check_boxes[v3_instance_id=' + inst_id  + ']').prop('checked', false); 
      $('.edit_instance_check_boxes[v3_instance_id=' + inst_id  + ']').prop('disabled', true); 
    } else if (this.checked === false) {
      $('.edit_instance_check_boxes[v3_instance_id=' + inst_id  + ']').prop('disabled', false); 
    };
  });
  ...
  $('.instanceLogButton').click(function(){
    ...
    $.getJSON("/v3_instances/" + inst_id + "/install_log", function(result){
  ...
  $(".v3_console_link").click(function(e){
    ...
    $.ajax({
      type: "GET",
      url: "/v3_instances/" + inst_id + "/console",
      async: false,
      //dataType: "json",
